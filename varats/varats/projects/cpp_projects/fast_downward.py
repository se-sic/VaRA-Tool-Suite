...
from benchbuild.command import ArgsToken, SourceRoot, WorkloadSet
...
from varats.utils.git_util import FullCommitHash, GitFileSource, ShortCommitHash
...
class _FDParameterRenderer:
    ...
    def __render_option(
        self, options: dict[str, int | bool], to_render: str
    ) -> str:
    ...
    def _render_lm_factory_arg(
        self, options: dict[str, int | bool], arg: str
    ) -> str:
        val = options.pop(arg, False)
        return f"{self.STR_REPRESENTATIONS[arg]}={str(val).lower()}"

    def _render_lm_count(self, options: dict[str, int | bool]) -> str:
    ...
    def _render_iPDB(self, options: dict[str, int | bool]) -> str:
    ...
    def _render_merge_and_shrink(self, options: dict[str, int | bool]) -> str:
    ...
    def _render_heuristic(self, options: dict[str, int | bool]) -> str:
        rendered = ""
    ...
    def render_internal(self, requested_options):
        options_dict: dict[str, int | bool] = {}
    ...
    def rendered(self, project: VProject, **kwargs: tp.Any) -> str:
        if not isinstance(project, FastDownward):
            raise NotImplementedError(
                "This renderer must not be used for any project other than FastDownward"
            )
...
class FastDownward(VProject, ReleaseProviderHook):
    ...
    def compile(self) -> None:
        ...
        translate = local[
            version_source / "builds/release/bin/translate/translate.py"
        ]
