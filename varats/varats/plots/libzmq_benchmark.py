...
class LibZQMBenchmarkPlot(Plot, plot_name="libzmq_benchmark"):
    ...
    def __extract_data(self, multi_wl_report: MultiWLAggregate,
                       hwm: int) -> tp.List[tp.Dict[str, tp.Any]]:
        data_rows = []
        for workload in multi_wl_report.keys():
            for text_report in multi_wl_report[workload]:
                repetition = text_report.filename.filename.split("_")[-1].split(
                    '.'
                )[0]
                new_row = {"hwm": hwm, "rep": repetition}
                with open(text_report.path, "r") as report_file:
                    for line in report_file:
                        if "message size" in line:
                            new_row["message_size"] = int(
                                line.split(" ")[2].strip()
                            )
                        if "count" in line:
                            new_row["count"] = int(line.split(" ")[2].strip())
                        if "latency" in line:
                            new_row["latency"] = float(
                                line.split(" ")[2].strip()
                            )
                        if "throughput" in line and "msg/s" in line:
                            new_row["throughput_msg"] = float(
                                line.split(" ")[2].strip()
                            )
                        if "throughput" in line and "Mb/s" in line:
                            new_row["throughput_mb"] = float(
                                line.split(" ")[2].strip()
                            )
    ...
    def load_benchmark_data(self) -> pandas.DataFrame:
        ...
        for base_name in report_data.get_baseline_names():
            ...
            for patch_name in report_data.get_patch_names():
                patch_report = report_data.get_specific_patched_report(
    ...
    def plot(self, view_mode: bool) -> None:
        ...
        mean_data = benchmark_data.groupby(['hwm', 'message_size'],
                                           as_index=False).agg({
                                               'throughput_mb': 'mean',
                                               'throughput_msg': 'mean',
                                               'latency': 'mean',
        ...
        reference_throughput = reference_df.set_index("message_size"
                                                     )['throughput_mb']

        mean_data['relative_throughput'] = mean_data.apply(
            lambda row: row['throughput_mb'] / reference_throughput[row[
                'message_size']],
            axis=1
        )
        ...
        for hwm in hwms:
            message_data = mean_data[mean_data['hwm'] == hwm].sort_values(
                by='message_size'
            )
            if hwm == 1000:
                plt.scatter(
                    message_data['message_size'],
                    message_data['throughput_mb'],
                    label=f'HWM: {int(hwm)} (Default)',
                    marker='*'
                )
                continue
...
class LibZMQBenchmarkPlotGenerator(
    PlotGenerator, generator_name="libzmq-benchmark", options=[]
):
