...
class ArchitectureTaintResultFunctionEntry:
    ...
    def create_architecture_taint_result_function_entry(
        name: str, raw_function_entry: dict[str, tp.Any]
    ):
...
class ArchitectureTaintReport(BaseReport, shorthand="ATR", file_type="yaml"):
    def __init__(self, path: Path) -> None:
        ...
        with open(path) as stream:
            ...
            for raw_func_entry in raw_architecture_taint_report['result-map']:
                new_function_entry = ArchitectureTaintResultFunctionEntry.create_architecture_taint_result_function_entry(
                    raw_func_entry,
                    raw_architecture_taint_report['result-map'][raw_func_entry],
                )
    ...
    ) -> ArchitectureTaintResultFunctionEntry | None:
        """
    ...
    def function_entries(
        self,
    ) -> dict[str, ArchitectureTaintResultFunctionEntry]:
...
class FeatureArchitectureTaintIncommingRegions:
    def __init__(
        self, features: list[str], incommingRegions: dict[str, list[str]]
    ):
    ...
    @property
    def incommingRegions(self) -> dict[str, list[str]]:
        """List of found instruction blame-interactions."""
...
class FeatureArchitectureTaintResultFunctionEntry:
    def __init__(
        ...
        demangled_name: str,
        file_name: str,
        incommingRegions: list[FeatureArchitectureTaintIncommingRegions],
    ) -> None:
        self.__name = name
        self.__demangled_name = demangled_name
        self.__file_name = file_name
    ...
    @property
    def interactions(self) -> list[FeatureArchitectureTaintIncommingRegions]:
        """List of found instruction blame-interactions."""
        return self.__insts
...
class FeatureArchitectureTaintReport(
    ...
    def __init__(self, path: Path) -> None:
        ...
        with open(path) as stream:
            ...
            raw_architecture_taint_report = next(documents)
            for raw_func_entry in raw_architecture_taint_report['result-map']:
                new_function_entry = FeatureArchitectureTaintResultFunctionEntry.create_feature_architecture_taint_result_function_entry(
                    raw_func_entry,
                    raw_architecture_taint_report['result-map'][raw_func_entry],
                )
                self.__function_entries[new_function_entry.name] = (
    ...
    def function_entries(
        self,
    ) -> dict[str, FeatureArchitectureTaintResultFunctionEntry]:
        """List of found function blame-interactions."""
        return self.__function_entries
