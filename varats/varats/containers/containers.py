...
from benchbuild.utils.settings import get_number_of_jobs, to_yaml
...
from varats.utils.settings import bb_cfg, save_bb_config, vara_cfg
...
class ImageBase(Enum):
    ...
    def __init__(self, distro: Distro, version_number: int) -> None:
        """Creates a base image object."""
...
class StageBuilder:
    ...
    def __init__(
        self, base: ImageBase, stage: ImageStage, image_name: str
    ) -> None:
        """Creates a StageBuilder context manager."""
        self.__base = base
        self.__stage = stage
    ...
    def __enter__(self) -> 'StageBuilder':
        """Opens the stage builder for configuration."""
        return self
    ...
    def __exit__(
        ...
        """Builds the image from this stage builder."""
...
# yapf: disable
_BASE_IMAGES: dict[ImageBase, tp.Callable[[StageBuilder], None]] = {
    ImageBase.DEBIAN_10:
...
def _create_container_image(
    ...
    Only rebuilds the given stage and subsequent stages.
    ...
    Args:
        ...
        force_rebuild: forces a rebuild of the complete base image
    Returns:
...
def _get_installable_research_tool() -> ContainerInstallable | None:
...
def _add_benchbuild_config(
    image_context: StageBuilder, env: dict[str, list[str]]
) -> None:
    bb_env: dict[str, list[str]] = defaultdict(list)
    for key, value in env.items():
        bb_env[key].extend(value)

    # copy libraries to image if LD_LIBRARY_PATH is set
    if "LD_LIBRARY_PATH" in bb_cfg()["env"].value:
