...
from varats.data.reports.region_verification_report import RegionVerificationReport as FRR
...
from varats.experiment.wllvm import RunWLLVM, _create_default_bc_file_creation_actions, get_cached_bc_file_path, BCFileExtensions
from varats.report.report import BaseReport, FileStatusExtension as FSE, ReportSpecification
from varats.utils.settings import bb_cfg

"""
Implements the feature region verifier experiment.

The experiment analyses a project with with feature region analysis, and compares
the results of the dominator to the results of the if-region approach
"""

class FeatureRegionGeneration(actions.Step):  # type: ignore
  """ Analyse a project with VaRA and compare dominator and if-region created FeatureRegions """
...
class FeatureRegionVerificationExperiment(VersionExperiment):
  ...
  def actions_for_project(self, project: Project) -> tp.List[actions.Step]:
    ...
    project.compile = get_default_compile_error_wrapped(
      project, self.REPORT_SPEC.main_report, FeatureRegionGeneration.RESULT_FOLDER_TEMPLATE
    )

    analysis_actions = _create_default_bc_file_creation_actions(project,
      [
    ...
    analysis_actions.append(FeatureRegionGeneration(project, self.REPORT_SPEC))
