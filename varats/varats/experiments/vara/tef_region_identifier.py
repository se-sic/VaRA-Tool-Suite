...
class TEFFeatureIdentifier(FeatureExperiment, shorthand="TEFid"):
    ...
    def actions_for_project(self,
        ...
        patch_provider: PatchProvider = PatchProvider.get_provider_for_project(
            project
        )
        ...
        print(f"{patches=}")
        patches = patches["region_identifier", "perf_prec"]
        print(f"{patches=}")
        ...
        if project.name == "HyTeg":
            # Workaround for HyTeg to remove busy sleep patch
            analysis_actions.append(
                RevertPatch(
                    project, patch_provider.get_by_shortname("add_busy_sleep")
                )
            )
...
class AnalyzeMPRTEFTrace(ProjectStep):
    ...
    def __call__(self):
        ...
        result_dict["Baseline"] = get_feature_performance_from_tef_report(
            tef_report
        )
        ...
        for patch in report.get_patch_names():
            ...
            result_dict[f"PATCHED_{patch}"
                       ] = get_feature_performance_from_tef_report(tef_report)
