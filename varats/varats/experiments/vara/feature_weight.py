...
class FeatureWeight(OutputFolderStep):
    """Times the execution of all project example workloads."""

    NAME = "FeatureWeight"
    DESCRIPTION = "Analyse Output with -fvara-weight"
    ...
    def analyze(self, tmp_dir: Path) -> actions.StepResult:
        ...
        with local.cwd(self.project.builddir): # changing working directory
            for prj_command in workload_commands(
                self.project, self.__binary, [WorkloadCategory.EXAMPLE]
            ):
                pb_cmd = prj_command.command.as_plumbum(project=self.project) # in workload_commands each line of command is now represented by this

                run_report_name = tmp_dir / create_workload_specific_filename(
                    "weight_report", prj_command.command, self.__num, ".txt"
                )

                # trying to pipe the output of running the binary
                run_cmd = local[pb_cmd, f"> {run_report_name}"]
...
class WeightWorkloads(VersionExperiment, shorthand="WWL"):
    ...
    def actions_for_project(
        ...
        analysis_actions.append(
            ZippedExperimentSteps(
                result_filepath, [
                    FeatureWeight(project, rep_num, binary)
